...
class WorkoutParser {
    ...
    static parse(raw) {
        ...
        for (let i = 2; i < lines.length; i++) {
            ...
            if (/^set\s+\d+/i.test(line)) {
                if (!current) {
                    current = { name: 'Unknown Exercise', sets: [] };
    ...
    static _parseSetLine(line) {
        ...
        const wMatch = rest.match(/([\d.]+)\s*kg\b/i);
        if (wMatch) out.weight_kg = parseFloat(wMatch[1]);
        const dMatch = rest.match(/([\d.]+)\s*mi\b/i);
        if (dMatch) out.distance_mi = parseFloat(dMatch[1]);
        const dkmMatch = rest.match(/([\d.]+)\s*km\b/i);
        if (dkmMatch) out.distance_km = parseFloat(dkmMatch[1]);
        const durMatch = rest.match(/([\d.]+)\s*m\b(?!i)/i);
        if (durMatch) out.duration_min = parseFloat(durMatch[1]);
        const rMatch = rest.match(/(\d+)\s*reps?\b/i) || rest.match(/[x×]\s*(\d+)/i);
        if (rMatch) out.reps = parseInt(rMatch[1], 10);
        if (
            out.reps == null &&
            out.weight_kg == null &&
            out.distance_mi == null &&
            out.duration_min == null
        ) {
            const bare = rest.match(/^(\d+)$/);
            if (bare) out.reps = parseInt(bare[1], 10);
        }
        return out;
    }
    ...
    static summarize(parsed) {
        ...
        const totalVolumeKg = parsed.exercises.reduce(
            (sum, ex) =>
                sum +
                ex.sets.reduce((s, set) => (set.weight_kg && set.reps ? s + set.weight_kg * set.reps : s), 0),
            0
        );
        return {
...
class WorkoutTracker {
    ...
    setupEventListeners() {
        ...
        qs('workout-clear-paste-btn')?.addEventListener('click', () => {
            const ta = qs('workout-paste');
            if (ta) ta.value = '';
            this.hidePreview();
        });
        qs('workout-save-btn')?.addEventListener('click', () => this.handleSave());
        qs('workout-cancel-btn')?.addEventListener('click', () => this.hidePreview());
        qs('workout-exercise-select')?.addEventListener('change', e =>
            this.renderProgressionChart(e.target.value)
        );
    ...
    handleParse() {
        ...
        if (!raw.trim()) {
            showNotification('Paste a workout first', 'error');
            return;
        }
        try {
            ...
            showNotification('Could not parse: ' + err.message, 'error');
        }
    }
    ...
    renderPreview(parsed) {
        ...
        document.getElementById('workout-preview-duration').textContent = parsed.duration_minutes
        ...
        if (exList) {
            exList.innerHTML = parsed.exercises
                .map(
                    ex => `
                        ...
                        <div class="workout-sets">
                            ${
                                ...
                                <div class="workout-set-row">
                                    ...
                                    .join('') ||
                                '<div class="workout-set-row"><span class="workout-set-detail">No sets</span></div>'
                            }
                        </div>
                    </div>`
                )
    ...
    async handleSave() {
        ...
        try {
            ...
            this.hidePreview();
            const ta = document.getElementById('workout-paste');
            if (ta) ta.value = '';
            await this.loadWorkouts();
            this.renderAll();
    ...
    async createTimeLogForWorkout(workout, durationMin) {
        ...
        const payload = {
            activity: workout.workout_type,
            category: 'Health',
            start_time: startDate.toISOString(),
            end_time: endDate.toISOString(),
            duration_seconds: durationMin * 60,
            date: workout.date,
            notes: 'Auto-logged from workout'
        };
        try {
            await API.addTimeLog(payload);
        } catch (err) {
            console.warn('Auto-timelog failed:', err);
        }
    }
    ...
    renderWeeklyStats() {
        ...
        const weekly = this.workouts.filter(
            w => w.date >= toDateStr(weekAgo) && w.date <= toDateStr(today)
        ).length;
        ...
        while (byDate.has(toDateStr(cur))) {
            streak++;
    ...
    renderHistory() {
        ...
        if (!this.workouts.length) {
            list.innerHTML = '<div class="loading">No workouts yet. Paste one above to get started.</div>';
            return;
        }
            ...
            .map(w => {
                ...
                const totalVolume = (w.exercises || []).reduce(
                    (sum, ex) =>
                        sum +
                        (ex.sets || []).reduce(
                            (s, set) => (set.weight_kg && set.reps ? s + set.weight_kg * set.reps : s),
                            0
                        ),
                    0
                );
    ...
    confirmDelete(id) {
        showGenericConfirm('Delete Workout?', 'This workout will be permanently removed.', () =>
    ...
    async renderProgressionChart(exerciseName) {
        ...
        if (!points.length) {
            showNotification('No weight data for this exercise yet.', 'error');
            return;
        }
