...
class TimeTracker {
    ...
    setupEventListeners() {
        ...
        qs('manual-clear-btn')?.addEventListener('click', () => {
            ...
            const md = qs('manual-date');
            if (md) md.value = this.viewDate;
        });
    ...
    async restoreActiveTimer() {
        ...
        try {
            ...
            console.warn('restoreActiveTimer remote failed', err);
        }
    }
    ...
    async handleStartTimer(e) {
        ...
        if (!activity || !CATEGORIES.includes(category)) {
            showNotification('Enter an activity and category', 'error');
            return;
        }
    ...
    async startTimer(activity, category) {
        if (this.activeTimer) {
            showNotification('A timer is already running. Stop it first.', 'error');
            return;
        }
        const now = Date.now();
    ...
    async stopTimer() {
        ...
        try {
            ...
            showNotification('Error saving time log: ' + err.message, 'error');
        }
    }
    ...
    async clearActiveTimerState() {
        ...
        try {
            ...
            console.warn('clearActiveTimer remote failed', err);
        }
    }
    ...
    updateElapsedDisplay() {
        ...
        el.textContent = formatDuration(
            Math.floor((Date.now() - this.activeTimer.start_epoch_ms) / 1000),
            false
        );
    }
    ...
    renderActiveTimerCard() {
        ...
        if (startedEl)
            startedEl.textContent = new Date(start_epoch_ms).toLocaleTimeString([], {
                hour: '2-digit',
                minute: '2-digit'
            });
    ...
    renderDailyStats(dayLogs) {
        ...
        dayLogs.forEach(l => {
            if (totals[l.category] !== undefined) totals[l.category] += Number(l.duration_seconds);
        });
        ...
        const score = Math.min(
            100,
            Math.round(((totals.Work + totals.Health) / Math.max(1, 24 * 3600 - totals.Sleep)) * 100)
        );
    ...
    renderTimeline(dayLogs) {
        ...
        if (!container) return;
        if (!dayLogs.length) {
            container.innerHTML = '<div class="loading">No activity logged for this day</div>';
            return;
        }
        const dayStart = new Date(`${this.viewDate}T00:00:00`);
        const totalMs = 24 * 3600 * 1000;
        const segments = dayLogs
            .map(l => {
        ...
        for (let h = 0; h <= 24; h += 6)
            ticks += `<div class="timeline-tick" style="left:${(h / 24) * 100}%;"><span>${String(h).padStart(2, '0')}:00</span></div>`;
    ...
    renderLogList() {
        ...
        if (!dayLogs.length) {
            list.innerHTML = '<div class="loading">No entries for this day</div>';
            return;
        }
        list.innerHTML = dayLogs
            .map(l => {
                const color = CATEGORY_COLORS[l.category] || '#9490c8';
                const icon = CATEGORY_ICONS[l.category] || '📁';
                const s = new Date(l.start_time),
                    e = new Date(l.end_time);
    ...
    confirmDelete(id) {
        showGenericConfirm('Delete Time Log?', 'This entry will be permanently removed.', () =>
            this.deleteLog(id)
        );
    }
        ...
        const datasets = CATEGORIES.map(cat => ({
            ...
            data: days.map(
                d =>
                    +(
                        this.logs
                            .filter(l => l.date === d && l.category === cat)
                            .reduce((s, l) => s + Number(l.duration_seconds), 0) / 3600
                    ).toFixed(2)
            ),
    ...
    getCategoryTotalsForDate(dateStr) {
        const totals = { Work: 0, Health: 0, Personal: 0, Leisure: 0, Sleep: 0 };
        this.getLogsForDate(dateStr).forEach(l => {
            if (totals[l.category] !== undefined) totals[l.category] += Number(l.duration_seconds);
        });
        return totals;
    }
